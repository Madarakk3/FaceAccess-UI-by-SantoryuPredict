          ...
          axios
            ...
            .then((response) => {
              ...
              if (response.data.registration_status == 200) {
                alert("Вы успешно зарегистрировались в системе SantoryuPredict ✅");
              }
            })
            .catch((error) => {
        ...
        axios
          ...
          .then((response) => {
            ...
            if (response.data.match_status === true) {
              alert("Добро пожаловать в систему SantoryuPredict: 🫡 " + response.data.user + " !");
            } else {
            if (response.data.user === "spoof_detected") {
              alert("Как тебе не стыдно? Ты подделка! 🤡");
            } else {
              alert("                       Неизвестный пользователь!                                    📝Пожалуйста, попробуйте заново или зарегистрируйте нового пользователя!📝");
             }
            }
      ...
      canvasRef.current.toBlob((blob) => {
        ...
        axios
          ...
          .then((response) => {
            console.log(response.data);
            if (response.data.match_status == true) {
              alert("Вы покинули систему SantoryuPredict:👋 " + response.data.user + " !");
            } else {
              alert("                       Неизвестный пользователь!                                    📝Пожалуйста, попробуйте заново или зарегистрируйте нового пользователя!📝");
            }
          })
